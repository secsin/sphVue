<template>
  <div class="register-container">
    ...
    <div class="register">
      <h3>
        注册新用户
        <span class="go">我有账号，去 <a target="_blank">登陆</a> </span>
      </h3>
      <div class="content">
        <label>手机号:</label>
        <input
...
export default {
  name: "Register",
  data() {
    return { phone: "", code: "", password: "", password1: "", agree: true };
  },
  methods: {
    async getCode() {
      const { phone } = this;
    ...
    async userRegister() {
      ...
      if (success) {
        try {
          await this.$store.dispatch("userRegister", { phone: this.phone, code: this.code, password: this.password, password1: this.password1 });
